...
export const deleteUser = async (req, res, next) => {
    console.log('User ID to delete:', req.params.userId); // Add logging
    ...
    try {
        ...
        if (!user) {
            return next(errorHandler(404, 'User not found')); // Ensure user exists
        }
...
export const getUsers = async (req, res, next) => {
    ...
    try {
        ...
        const now = new Date();
        const oneMonthAgo = new Date(now.setMonth(now.getFullYear(),now.getMonth() - 1,
    now.getDate()));
        ;const lastMonthsUsers = await User.countDocuments({createdAt:{$gte:oneMonthAgo}});
         res.status(200).json({totalUsers,users:userWithoutPassword,lastMonthsUsers});
    } catch (error) {
        next(error);
    }
}
